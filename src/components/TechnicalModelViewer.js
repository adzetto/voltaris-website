    ...
    return Object.keys(technicalSpecs).map((key) => {
      ...
      return (
        <group key={key} position={position}>
          ...
          >
            <div className="w-48 bg-black/90 backdrop-blur-md p-2 border border-red-500/30 rounded text-xs">
              <div className="text-red-500 font-bold mb-1">{spec.title}</div>
...
const TechnicalModelViewer = () => {
  ...
  return (
    <div className="relative w-full h-52 sm:h-64 md:h-96 lg:h-[420px] rounded-lg overflow-hidden border border-gray-800 shadow-lg shadow-black/50 model-viewer-container">
      {/* Professional UI overlay panel */}
      <div className="absolute left-0 right-0 top-0 z-20 bg-gradient-to-b from-black/90 to-transparent">
        <div className="flex flex-col xs:flex-row items-start xs:items-center justify-between px-2 xs:px-3 py-2 border-b border-gray-800/30">
          <div className="flex items-center flex-wrap gap-2 xs:gap-3 mb-1 xs:mb-0">
            <div className="flex items-center px-2 py-1 bg-black/80 border-l-2 border-red-500/60 rounded-r status-indicator">
              <div className="h-1.5 w-1.5 rounded-full bg-red-500 mr-2 animate-pulse"></div>
              <span className="text-xs font-mono tracking-wide text-gray-300 uppercase">
                Model: <span className="text-red-400 font-medium">Active</span>
              </span>
            </div>
            
            <div className="hidden sm:flex items-center">
              <span className="text-xs font-mono tracking-wide text-gray-300 uppercase">
                VOL-1 <span className="text-red-400 font-medium">ELEKTROMOBİL</span>
              </span>
            </div>
          </div>
        
          <div className="flex space-x-2 xs:space-x-3 text-xs font-mono">
            <div className="flex items-center px-2 py-1 bg-black/80 border-r-2 border-blue-500/60 rounded-l">
              <div className="h-1.5 w-1.5 rounded-full bg-blue-500 mr-1.5"></div>
              <span className="text-gray-300 uppercase text-[10px] xs:text-xs">Render: <span className="text-blue-400">RT</span></span>
            </div>
            <div className="flex items-center px-2 py-1 bg-black/80 border-r-2 border-green-500/60 rounded-l">
              <div className="h-1.5 w-1.5 rounded-full bg-green-500 mr-1.5"></div>
              <span className="text-gray-300 uppercase text-[10px] xs:text-xs">Ölçek: <span className="text-green-400">1:32</span></span>
            </div>
          </div>
        </div>
      </div>

      {/* Professional UI overlay panel - REMOVED duplicate render info */}
      <div className="absolute left-0 right-0 bottom-0 z-20 bg-gradient-to-t from-black/90 to-transparent">
        <div className="flex flex-col xs:flex-row justify-between items-start xs:items-center gap-2 px-2 xs:px-3 py-2 border-t border-gray-800/30">
          <div className="flex flex-wrap items-center gap-2 xs:gap-3 text-[10px] xs:text-xs font-mono">
            <div className="flex items-center px-2 py-1 bg-black/80 border-l-2 border-blue-500/60 rounded-r">
              <div className="h-1.5 w-1.5 rounded-full bg-blue-500 mr-1.5"></div>
              <span className="text-gray-300 uppercase">Zoom: <span className="text-blue-400">Aktif</span></span>
            </div>
            <div className="flex items-center px-2 py-1 bg-black/80 border-l-2 border-red-500/60 rounded-r">
              <div className="h-1.5 w-1.5 rounded-full bg-red-500 mr-1.5"></div>
              <span className="text-gray-300 uppercase">Döndür: <span className="text-red-400">Manuel</span></span>
            </div>
          </div>
          
          <button
            className="text-[10px] xs:text-xs font-mono px-2 xs:px-3 py-1 xs:py-1.5 bg-black/80 border border-gray-700 text-gray-300 rounded hover:border-red-500/50 hover:text-red-400 transition-colors flex items-center model-controls"
            onClick={() => {
              console.log("Center button clicked");
              if (modelViewerRef.current) {
                modelViewerRef.current.resetCameraView();
              } else {
                console.error("Model viewer reference not available");
                // Try dispatching an event as a fallback method
      ...
      {/* Corner accent lines */}
      <div className="absolute top-0 left-0 w-8 sm:w-16 h-px bg-gradient-to-r from-red-500/30 to-transparent"></div>
      <div className="absolute top-0 left-0 h-8 sm:h-16 w-px bg-gradient-to-b from-red-500/30 to-transparent"></div>
      <div className="absolute top-0 right-0 w-8 sm:w-16 h-px bg-gradient-to-l from-blue-500/30 to-transparent"></div>
      <div className="absolute top-0 right-0 h-8 sm:h-16 w-px bg-gradient-to-b from-blue-500/30 to-transparent"></div>
      <div className="absolute bottom-0 left-0 w-8 sm:w-16 h-px bg-gradient-to-r from-blue-500/30 to-transparent"></div>
      <div className="absolute bottom-0 left-0 h-8 sm:h-16 w-px bg-gradient-to-t from-blue-500/30 to-transparent"></div>
      <div className="absolute bottom-0 right-0 w-8 sm:w-16 h-px bg-gradient-to-l from-red-500/30 to-transparent"></div>
      <div className="absolute bottom-0 right-0 h-8 sm:h-16 w-px bg-gradient-to-t from-red-500/30 to-transparent"></div>
      
      <div className="h-full w-full z-10 overflow-hidden">
        <EnhancedModelViewer ref={modelViewerRef} />
      </div>
      
      {/* Bottom right status - REMOVED as requested */}
    </div>
  );
};
