...
export const TeamMemberCard = ({ member, index, highlight = false }) => {
  ...
  return (
    <div 
      className={`team-card relative bg-black/80 p-5 rounded-lg overflow-hidden transition-all duration-300 ${highlight ? 'ring-2 ring-offset-2 ring-offset-black ring-blue-500/50' : ''}`}
      style={{ 
        boxShadow: highlight ? '0 0 15px rgba(59, 130, 246, 0.3)' : 'none'
      }}
    >
      {/* Bordered image container */}
      <div className="relative w-full h-64 mb-5 rounded-xl overflow-hidden border-2 border-gray-800 group">
        <img 
      ...
      <p className="text-sm text-blue-400 mb-3">{member.department}</p>
      ...
      <div className="flex space-x-2 mt-4">
        {member.linkedin && (
          <a 
            ...
            className="p-2 bg-gray-900 hover:bg-blue-900/20 rounded-full text-gray-400 hover:text-blue-400 transition-colors"
          >
            <Linkedin size={16} />
          </a>
        )}
        {member.email && (
          <a 
            ...
            className="p-2 bg-gray-900 hover:bg-red-900/20 rounded-full text-gray-400 hover:text-red-400 transition-colors"
          >
            <Mail size={16} />
          </a>
        )}
        {member.github && (
          <a 
            ...
            target="_blank" 
            rel="noopener noreferrer"
            className="p-2 bg-gray-900 hover:bg-purple-900/20 rounded-full text-gray-400 hover:text-purple-400 transition-colors"
          >
            <Github size={16} />
          </a>
        )}
...
const TimelineSection = ({ 
  ...
  return (
    <div className="relative flex flex-col md:flex-row items-center md:justify-start w-full min-w-0 overflow-visible">
      ...
      <div className="absolute left-1/2 transform -translate-x-1/2 w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 rounded-full bg-black border-2 border-gray-800 flex items-center justify-center z-20">
      ...
      <div className={`min-w-0 max-w-full relative px-2 xs:px-3 sm:px-4 md:px-0 ${mobileLayoutClasses}`}>
        ...
        {position !== 'center' && (
          <div className={`absolute top-1/2 hidden md:block ${position === 'left' ? 'right-0 translate-x-1/2' : 'left-0 -translate-x-1/2'} transform -translate-y-1/2 w-16 h-px bg-gradient-to-${position === 'left' ? 'r' : 'l'} from-${color}-500/80 to-gray-800/20`}></div>
        )}
        ...
        <div className={`bg-black/60 backdrop-blur-md rounded-lg border border-${color}-500/30 p-2 xs:p-3 sm:p-4 md:p-5 shadow-lg mt-5 md:mt-0 w-full min-w-0`}>
          {/* Captain highlight if exists */}
          {captain && (
            <div className="mb-3 pb-3 md:mb-4 md:pb-4 border-b border-gray-800">
              ...
              <div className="flex items-center">
                <div 
                  className="relative h-8 w-8 xs:h-10 xs:w-10 md:h-12 md:w-12 rounded-full border-2 border-gray-800 overflow-hidden mr-2 sm:mr-3 cursor-pointer hover:border-blue-500 transition-colors"
                ...
                <div className="max-w-[calc(100%-45px)] xs:max-w-[calc(100%-60px)] md:max-w-[calc(100%-70px)]">
                  <div className="font-bold text-white text-xs xs:text-sm md:text-base truncate">{captain.name}</div>
                  <div className="text-xs text-blue-400 truncate">{captain.role}</div>
                </div>
                
                {/* Detail card - adaptive positioning for all screen sizes */}
                {activeCard === 'captain' && (
                  <div className={`absolute left-0 right-0 md:inset-x-auto ${position === 'center' || position === 'right' ? 'md:left-full md:right-auto md:ml-3' : 'md:right-full md:left-auto md:mr-3'} top-full md:top-0 z-30 w-full md:w-64 mt-2 md:mt-0 bg-black/90 backdrop-blur-lg p-3 rounded-lg border border-blue-500/30 shadow-xl transform transition-all duration-300`}>
                    <div className="flex items-start space-x-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="font-bold text-white mb-1 truncate">{captain.name}</div>
                        <div className="text-xs text-blue-400 mb-1 sm:mb-2 truncate">{captain.role}</div>
                        <div className="text-xs text-gray-400 truncate">{captain.department}</div>
                        
                        <div className="flex space-x-2 mt-2">
                          {captain.linkedin && (
                            <a href={captain.linkedin} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-blue-400">
                              <Linkedin size={12} className="sm:w-3.5 sm:h-3.5 md:w-4 md:h-4" />
                            </a>
                          )}
                          {captain.email && (
                            <a href={`mailto:${captain.email}`} className="text-gray-400 hover:text-red-400">
                              <Mail size={12} className="sm:w-3.5 sm:h-3.5 md:w-4 md:h-4" />
                            </a>
                          )}
                          {captain.github && (
                            <a href={captain.github} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-purple-400">
          ...
          <div>
            {subunits.length > 0 ? (
              ...
              <div className="space-y-2 sm:space-y-3 md:space-y-4">
                {subunits.map((subunit, idx) => (
                  <div key={idx} className="mb-2 md:mb-3">
                    <div 
                      className={`flex items-center text-2xs xs:text-xs md:text-sm text-${color}-400 p-1.5 xs:p-2 rounded hover:bg-gray-900/30 cursor-pointer transition-colors`}
                    ...
                    <div className={`flex flex-wrap ${activeCard === `subunit-header-${idx}` ? 'block' : 'md:block hidden'}`}>
                      {subunit.members?.map((member, i) => (
                        <div 
                          ...
                          onMouseLeave={() => window.innerWidth >= 768 && setActiveCard(null)}
                        >
                          <div className="h-6 w-6 xs:h-8 xs:w-8 sm:h-10 sm:w-10 rounded-full border border-gray-800 overflow-hidden cursor-pointer hover:border-blue-500 transition-colors flex-shrink-0">
                            <img 
                          ...
                          {activeCard === `${idx}-${i}` && (
                            <div className={`fixed inset-x-2 xs:inset-x-4 md:absolute md:inset-x-auto ${position === 'center' || position === 'right' ? 'md:left-full md:ml-3' : 'md:right-full md:mr-3'} bottom-16 xs:bottom-20 md:bottom-auto md:top-0 z-40 md:z-30 w-auto md:w-60 bg-black/90 backdrop-blur-lg p-2 xs:p-3 rounded-lg border border-blue-500/30 shadow-xl transform transition-all duration-300`}>
              ...
              // Render as single group
              <div>
                <div className="text-2xs xs:text-xs md:text-sm text-gray-400 mb-2 md:mb-3">
                  Takım Üyeleri ({members?.length || 0})
                </div>
                <div className="flex flex-wrap">
                  {members?.map((member, i) => (
                    <div 
                      key={member.id || i} 
                      className="relative mr-1 mb-1"
                      onClick={() => handleMemberInteraction(`member-${i}`)}
                      onMouseEnter={() => window.innerWidth >= 768 && setActiveCard(`member-${i}`)}
                      onMouseLeave={() => window.innerWidth >= 768 && setActiveCard(null)}
                    >
                      <div className="h-6 w-6 xs:h-8 xs:w-8 sm:h-10 sm:w-10 rounded-full border border-gray-800 overflow-hidden cursor-pointer hover:border-blue-500 transition-colors flex-shrink-0">
                        <img 
  ...
  return (
    <div className="relative">
      <div className="flex items-start space-x-2 xs:space-x-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="font-bold text-white mb-0.5 xs:mb-1 text-xs xs:text-sm md:text-base truncate">{member.name}</div>
          <div className="text-2xs xs:text-xs text-blue-400 mb-0.5 xs:mb-1 truncate">{member.role}</div>
          <div className="text-2xs xs:text-xs text-gray-400 truncate">{member.department}</div>
          ...
          <div className="flex space-x-2 mt-1 xs:mt-2">
            ...
            {member.email && (
              <a href={`mailto:${member.email}`} className="text-gray-400 hover:text-red-400">
                <Mail size={12} className="xs:w-3.5 xs:h-3.5 md:w-4 md:h-4" />
              </a>
            )}
            {member.github && (
              <a href={member.github} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-purple-400">
...
const MemberDetailCard = ({ member }) => {
  return (
    <div className="absolute top-0 left-full ml-3 z-30 w-60 bg-black/90 backdrop-blur-lg p-3 rounded-lg border border-blue-500/30 shadow-xl transform transition-all duration-300">
      <div className="flex items-start space-x-3">
...
export const TeamStats = () => {
  const stats = [
    ...
    { label: "Prototip Geliştirme", value: 1, color: "red" },
    { label: "Kuruluş", value: 2024, color: "blue", unit: "" }
  ];
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-2 xs:gap-3 md:gap-4 px-2 xs:px-4 md:px-0">
      {stats.map((stat, index) => (
        <div 
          key={index} 
          className="bg-black/80 border border-gray-800 rounded-lg p-2 xs:p-3 md:p-4 relative overflow-hidden"
        >
          ...
          <div className="relative z-10">
            <div className={`text-xl xs:text-2xl md:text-3xl font-bold text-${stat.color}-500`}>
              {stat.value}{stat.unit && <span className="text-2xs xs:text-xs md:text-sm ml-0.5 xs:ml-1">{stat.unit}</span>}
            </div>
            <div className="text-2xs xs:text-xs md:text-sm text-gray-400">{stat.label}</div>
            
            {/* Progress indicator */}
            <div className="mt-1 xs:mt-2 w-full h-1 bg-gray-800 rounded-full overflow-hidden">
              <div 
...
export const SponsorShowcase = ({ sponsors }) => {
  return (
    <div className="sponsor-showcase rounded-lg bg-black/70 backdrop-blur-sm border border-gray-800 overflow-hidden p-4 mx-auto max-w-3xl">
            ...
            >
              ...
              >
                <div className={`relative h-24 w-40 grayscale hover:grayscale-0 transition-all duration-500 flex items-center justify-center sponsor-tier-${sponsor.tier}`}>
                  ...
                  />
                  <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent opacity-0 group-hover:opacity-100 transform -skew-x-12 translate-x-full group-hover:translate-x-0 transition-all duration-700"></div>
                </div>
                <p className="text-center text-xs text-gray-400 mt-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">{sponsor.name}</p>
              </a>
            </div>
          ))}
        </div>
      ...
      <div className="mt-4 sponsor-circuit-decoration">
        <div className="flex items-center justify-center">
          <div className="h-px w-full bg-gradient-to-r from-red-500/30 via-transparent to-blue-500/30"></div>
        </div>
      </div>
    </div>
  );
