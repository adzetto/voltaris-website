  ...
  return (
    <div className="bg-black/80 p-6 rounded-lg border border-gray-800 relative overflow-hidden">
      ...
      <form onSubmit={handleSubmit} className="space-y-4 relative z-10">
        ...
        <div>
          <label className="block text-gray-400 text-sm mb-1">İlgilendiğiniz Sponsorluk Paketi <span className="text-red-500">*</span></label>
          <select 
            name="package"
        ...
        {/* Status messages */}
        {status.submitted && (
          <div className={`p-3 rounded-lg text-sm ${status.success ? 'bg-green-900/50 text-green-400 border border-green-900' : 'bg-red-900/50 text-red-400 border border-red-900'}`}>
            {status.success 
              ? 'Sponsorluk talebiniz başarıyla gönderildi. En kısa sürede size dönüş yapacağız.'
              : status.error}
          </div>
        )}
        ...
        <div>
          <button 
            type="submit" 
            className={`bg-gradient-to-r from-red-600 to-red-800 hover:from-red-500 hover:to-red-700 text-white px-6 py-3 rounded-lg flex items-center justify-center font-medium transition-colors shadow-lg shadow-red-900/20 ${status.submitting ? 'opacity-70 cursor-not-allowed' : ''}`}
